/**
 * Classement partagé du Quiz Simandou — Google Apps Script.
 ...
 * La feuille reçoit une ligne par résultat : date, nom, score, total, mode.
 */
const SHEET = 'Scores';
...
function sheet_() {
  ...
  if (!sh) { sh = ss.insertSheet(SHEET); sh.appendRow(['date', 'name', 'ok', 'tot', 'mode']); }
  return sh;
}
...
function doPost(e) {
  try {
    ...
    sheet_().appendRow([new Date(), name, ok, tot, String(d.mode || '').slice(0, 40)]);
    lock.releaseLock();
